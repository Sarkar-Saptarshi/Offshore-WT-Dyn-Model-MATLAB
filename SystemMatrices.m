function [IM, f, Controls] = SystemMatrices(t, q, Controls, DOFs, ElastoDyn, Airfoils, Twr, Bld, Platform, Wind, WindNom, wave, mooring_load_ptr, Servo, TMDI)
%#Codegen
ep = 1e-15;
OnePlusEps = 1+ep;
persistent LastTime
...
% WindNom.velocity(:) = interpn(Wind.t_TurbSim, [1 2 3], Wind.y, Wind.z, Wind.velocity, t, [1 2 3], Wind.y, Wind.z);

qr = {t, [1 2 3], Wind.y, Wind.z};
WindNom.velocity(:) = Wind.Velocity(qr);
% if t > 0
%     disp(WindNom.velocity(1,:,:))
%     pause
% end
...
%% Get TMDI matrices 
[C_TMDI, f_TMDI] = TMDISystemMatrix_mex(Comq, q_Nom, Controls, ElastoDyn, Twr, Bld, TMDI);
